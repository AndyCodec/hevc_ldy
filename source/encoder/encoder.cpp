...
{
    //printf("setAnalysisDataAfterZScan \n");
...
void LookEncoder::fetchStats(x265_stats *stats, size_t statsSizeBytes)
{
    if (statsSizeBytes >= sizeof(stats))
    {
        stats->globalPsnrY = m_analyzeAll.m_psnrSumY;
...
bool LookEncoder::computeSPSRPSIndex()
{
        printf("computeSPSRPSIndex \n");
        RPS* rpsInSPS = m_sps.spsrps;
        int* rpsNumInPSP = &m_sps.spsrpsNum;
...
void LookEncoder::getStreamHeaders(NALList& list, Entropy& sbacCoder, Bitstream& bs)
{
    sbacCoder.setBitstream(&bs);
...
void LookEncoder::freeAnalysis(x265_analysis_data* analysis)
{
    if (m_param->bDisableLookahead && m_rateControl->m_isVbv)
    {
        X265_FREE(analysis->lookahead.satdForVbv);
