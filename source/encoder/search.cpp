...
void Search::codeIntraLumaQT(Mode& mode, const CUGeom& cuGeom, uint32_t tuDepth, uint32_t absPartIdx, bool bAllowSplit, Cost& outCost, const uint32_t depthRange[2])
{
    ...
    bool mightNotSplit  = log2TrSize <= depthRange[1]; //depthRange[0-1]: minTUSize-maxTUSize: 2-5
...
void Search::checkIntra(Mode& intraMode, const CUGeom& cuGeom, PartSize partSize)
{
    ...
    intraMode.initCosts();
    intraMode.lumaDistortion += estIntraPredQT(intraMode, cuGeom, tuDepthRange); //计算Y分量
    if (m_csp != X265_CSP_I400)
    {
        intraMode.chromaDistortion += estIntraPredChromaQT(intraMode, cuGeom); //计算uv分量
        intraMode.distortion += intraMode.lumaDistortion + intraMode.chromaDistortion; //失真
    }
    else
...
sse_t Search::estIntraPredQT(Mode &intraMode, const CUGeom& cuGeom, const uint32_t depthRange[2])
{
    ...
    for (uint32_t puIdx = 0; puIdx < numPU; puIdx++, absPartIdx += qNumParts)
    {
        ...
        else
        {
            ...
            //printf("==>numPU = %d,\t maxCandCount = %d\n", numPU, maxCandCount);
            /* measure best candidates using simple RDO (no TU splits) */
            bcost = MAX_INT64;
            for (int i = 0; i < maxCandCount; i++) //循环2+3+depth/2
            {
                ...
                if (checkTransformSkip)
                    ...
                    codeIntraLumaQT(intraMode, cuGeom, initTuDepth, absPartIdx, false, icosts, depthRange);//循环maxCandCount次， 倒数第三个参数为false
        ...
        if (checkTransformSkip)
            ...
            codeIntraLumaQT(intraMode, cuGeom, initTuDepth, absPartIdx, true, icosts, depthRange); //第二次调用，倒数第三个参数为true
...
void Search::updateCandList(uint32_t mode, uint64_t cost, int maxCandCount, uint32_t* candModeList, uint64_t* candCostList)
{
    ...
    for (int i = 0; i < maxCandCount; i++) //从前N次中找出最大Cost的编号
    {
    ...
    if (cost < maxValue) //如果当前cost小于最大值，降最大值替换为当前cost，可保证前N次是最小的
    {
