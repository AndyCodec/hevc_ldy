...
uint32_t Quant::rdoQuant(const CUData& cu, int16_t* dstCoeff, TextType ttype, uint32_t absPartIdx, bool usePsy)
{
    ...
    /*
    1. 常规量化，获得初始的量化值。会执行全零判断，如果是系数全零，则直接返回，不再执行RDOQ过程。
    */
    ...
    if (!numSig) // 如果常规量化后系数为全零，则跳过RDOQ过程（这是RDOQ提前终止的快速算法）
        return 0;
    //FILE *out = fopen("E://myData//q265//quant_x265.txt", "wt");
    //for (int i = 0; i < numCoeff; i++)
    //{
    //    if (i % 32 == 0)
    //        fprintf(out, "\n");
    //    fprintf(out, "%d\t", (int)(dstCoeff[i]));
    //}
    //fclose(out);
...
#define PSYVALUE(rec)   ((psyScale * (rec)) >> X265_MAX(0, (2 * transformShift + 1)))
    ...
    int64_t costCoeff[trSize * trSize];   /* d*d + lambda * bits */ // 每一个系数花费 
    int64_t costUncoded[trSize * trSize]; /* d*d + lambda * 0    */ // 每一个系数被量化为0的花费（Z型顺序存储）
    int64_t costSig[trSize * trSize];     /* lambda * bits       */  // 每一个系数的是否为0标记(sig_coeff_flag)的花费
    ...
    int sigRateDelta[trSize * trSize];   /* signal difference between zero and non-zero */ // 将系数量化为0和量化为非0，系数标记（sig_coeff_flag）的花费差异
...
#if CHECKED_BUILD || _DEBUG
    // clean output buffer, the asm version of scanPosLast Never output anything after latest non-zero coeff group
    memset(coeffNum, 0, sizeof(coeffNum)); //置零
    memset(coeffSign, 0, sizeof(coeffNum)); //为啥是 sizeof(coeffNum)， 应该是 sizeof(coeffSign)，因为size都是MLS_GRP_NUM？
    memset(coeffFlag, 0, sizeof(coeffNum));
#endif
    ...
    {
        // non-psy path
        for (int cgScanPos = cgLastScanPos + 1; cgScanPos < (int)cgNum ; cgScanPos++) // 遍历每一个全零的CG，此处的全零CG为最后一个非零系数之后的CG
        {
            X265_CHECK(coeffNum[cgScanPos] == 0, "count of coeff failure\n"); // 再次确认是否该CG中非零系数的个数是0
    ...
    // step1. 对每个系数单独做RDO优化，找到率失真意义上的最优量化值
    for (int cgScanPos = cgLastScanPos; cgScanPos >= 0; cgScanPos--)// 从最后一个非零CG开始遍历每个CG，依次遍历每一个CG
    {
        ...
        //针对非零CG，先做处理
        // 如果当前CG不是第一个CG，并且CG系数为全零，即该全零CG的位置位于最后一个非零系数之前（前面处理的全零CG为最后一个非零系数之后的CG，两者位置不同）
        // 如果发现全零的CG，处理方法与上文中，最后一个非零CG之后的CG的处理方法相同；
        //但是对这些系数又计算了量化为0和非零时，系数标记的花费，这是为了之后进行step2、step3的优化
        ...
        for (int scanPosinCG = cgSize - 1; scanPosinCG >= 0; scanPosinCG--) // 扫描CG内部的每一个系数
        {
            ...
            // 如果当前扫描位置在最后一个非零系数之后，则将这些系数的cost都设置为0
            ...
            else// 如果当前位置位于最后一个非零系数之前，并且该系数非零，估计每一个系数的最优量化值
            {
                subFlagMask >>= 1;
                ...
                if ((int)scanPos == lastScanPos)
                    // 如果当前位置是最后一个非零系数的位置，则将量化为0和非0的系数标记（sig_coeff_flag）的花费差异设为0
                ...
                if ((cu.m_slice->m_pps->bSignHideEnabled ? ~0 : 0) & level) //符号位是否需要隐藏，在pps中判断
                {
        ...
        else if (sigCoeffGroupFlag64 & cgBlkPosMask) //当前CG非全零
        {
            ...
            if (costZeroCG < totalRdCost && m_rdoqLevel > 1) // m_rdoqLevel == 2，特殊处理？
            {
    ...
    /*
    该循环从第一个循环中找到的最后一个非零开始，然后通过在这个位置测量最后一个NZ的真实RD成本
    来细化最后一个非零。然后在所有前面的系数上计算RD代价，直到遇到一个系数大于1或者我们没有系数来
    计算为止。这将考虑到在最后一个NZ之前编写空组和空系数的代价。基本的最佳代价是CBF=0时的RD代价。
    简言之，就是重新确定最后一个非零系数的位置。
    */
    int  bestLastIdx = 0;
    bool foundLast = false;
    for (int cgScanPos = cgLastScanPos; cgScanPos >= 0 && !foundLast; cgScanPos--) //依次处理每个CG
    {
        if (!cgScanPos || cgScanPos == cgLastScanPos)//跳过第一个和最后一个非零CG
        {
        ...
        else if (sigCoeffGroupFlag64 & (1ULL << codeParams.scanCG[cgScanPos]))//若当前CG非全零
        {
